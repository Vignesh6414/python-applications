...
def transfer_money(mydb, mycursor, sender_customer_id):
    ...
    mycursor.execute("SELECT * FROM customers WHERE Name = %s", (receiver_name,))
    receiver_result = mycursor.fetchone()
    print(sender_customer_id)
    print(receiver_name)
    ...
    print(receiver_customer_id)

    mycursor.execute("SELECT id FROM customers WHERE id = %s", (sender_customer_id,))
    ...
    if sender_balance_result:
        ...
        amounts = float(input("Enter the amount to transfer: "))
        amount = []
        ...
        if sender_balance >= amount[0]:  
            try:
                ...
                mycursor.execute("UPDATE account SET AMOUNT = AMOUNT - %s WHERE customer_id = %s", (amount[0], sender_customer_id))


                mycursor.execute("UPDATE account SET AMOUNT = AMOUNT + %s WHERE customer_id = %s", (amount[0], receiver_customer_id))
                print("this is working")
